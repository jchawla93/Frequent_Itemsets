...
def generateFrequentItemsets(baskets, size, bitmap1, bitmap2, previousFrequentItemsets):
    ...
    for line in baskets:
        for subset in itertools.combinations(line, size):
            ...
            if bitmapLookup(bitmap1, list(subset), prime1) and bitmapLookup(bitmap2, list(subset),
                                                                            prime2) and count == len(list(temp)):
...
if __name__ == '__main__':
    support = int(4)
    bucketCount = int(5)
    inputfile = open("/Users/jiteshchawla/PycharmProjects/Frequent_Itemsets/input_sample.txt")
